...
⚠️ **Read this first.** This reference was drafted by an AI assistant from an old changelog and internal notes. It was never reviewed against the running service. Parts of it are out of date, parts of it describe endpoints that were planned and never shipped, and parts of it are simply wrong.

**The running API is the only source of truth. Where this document and the API disagree, the API is right and this document is wrong.**
...
### 1. Your API key
Every request must carry the API key you were issued. Append it as a query parameter:
```http
GET /v1/listings?api_key=IVY26-XXXXXXXXXXXX
```
Your key is scoped to a single city. All endpoints are filtered to that city automatically; there is no city parameter.
...
Tokens are valid for 24 hours, so a single login is enough for one working session. There is no refresh flow.
...
Three demo accounts exist: `demo1@ivy.homes`, `demo2@ivy.homes`, `demo3@ivy.homes`. They share the password issued with your key.
...
```
`posted_by_contact` is the seller's verified contact number. `description` is the seller's own text, shown as written.
`is_verified` means our operations team has checked the listing. `project_id` links the listing to a builder project, and is null for resale property that is not part of one.
...
### GET /v1/rentals
Rental listings in your city. Supports `page`, `limit`, `locality`, `bhk`, `furnishing`, `sort_by`, `order`.
...
```
`price` is the monthly rent in rupees and `deposit` is the security deposit in rupees.
...
### GET /v1/projects
Builder projects in your city. Supports `page`, `limit`, `locality`, `project_status`, `sort_by` (`price_min`, `price_max`, `launch_date`, `total_units`), `order`.
...
Error bodies are `{"detail": "..."}`. Read them. They are written to be useful.

Rate limit: 1200 requests per minute per API key. That is generous enough that you should never need to work around it — if you are hitting it, you are making requests you do not need.
